...
7. **Marcar el tipo de lección (\`kind\`)**: cada lección lleva \`kind: 'demo' | 'normal'\`. Usa \`'demo'\` cuando la clase es el instructor trabajando con las manos en un procedimiento sobre el animal (laparoscopía, inseminación, descolado, inyecciones, y similares) — en esas etapas posteriores de corte automático NO se recorta el silencio interno de la lección, porque el silencio ES el trabajo visible (el instructor operando, sin narrar). Usa \`'normal'\` para el resto, en particular cuando el instructor está hablando a cámara o frente a un pizarrón/objetos sin realizar un procedimiento manual sobre el animal (aunque muestre ingredientes u otros materiales narrando de forma continua). Ante la duda, usa \`'normal'\`.
...
- \`modules\`: la estructura propuesta (módulos → lecciones → segmentos con rangos de tiempo del clip fuente). Incluye ahí, como segmento final de la lección afín, cada clip con veredicto 'broll' (topic "B-roll: <qué se ve>"); no lo omitas de la estructura. Cada lección lleva su \`kind: 'demo' | 'normal'\` según la regla 7 de arriba.
